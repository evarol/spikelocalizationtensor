...
def main():
    ...
    figure, axes = plt.subplots(
        len(omega), 2, figsize=(11, 1.18 * len(omega) + 1.5),
        sharex="col", constrained_layout=True,
        gridspec_kw={"width_ratios": (3.4, 1.6)},
    )
    axes = np.atleast_2d(axes)
    for row, (waveform_axis, usage_axis) in enumerate(axes):
        color = colors[row]
        waveform_axis.plot(samples, omega[row], color=color, linewidth=1.3)
        waveform_axis.axhline(0, color="0.8", linewidth=0.6)
        waveform_axis.set_ylim(-1.05 * amplitude_limit, 1.05 * amplitude_limit)
        waveform_axis.set_ylabel(rf"$\Omega_{{{row}}}$")
        waveform_axis.grid(alpha=0.18)

        usage_axis.barh(0, fractions[row], color=color, height=0.58)
        usage_axis.text(
            fractions[row] + usage_limit * 0.018, 0,
            f"{100 * fractions[row]:.1f}%  ({counts[row]:,})",
    ...
    axes[-1, 1].set_xlabel("fraction of fitted spikes")
