...
def detect_spikes(recording_path, output_path):
    # pass full file path so the extractor can resolve the stream
    recording_path_obj = Path(recording_path)
    file_suffix = recording_path_obj.suffix.lower()

    if file_suffix == ".cbin":
        recording = se.read_cbin_ibl(recording_path, stream_name="ap")
    ...
    spikes = detect_peaks(recording, method="locally_exclusive")

    spike_times = np.array(spikes["sample_index"], dtype=np.int64)
    spike_channels = np.array(spikes["channel_index"], dtype=np.int64)
