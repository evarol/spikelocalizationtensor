...
def load_recording(recording_path: str) -> si.BaseRecording:
    ...
    if file_suffix == ".cbin":
        recording = se.read_cbin_ibl(file_path=recording_path, stream_name="ap")
        ...
        # SpikeGLX takes the recording path directly (same as detect_peaks)
...
def extract_neighborhoods(
    ...
    # PeakRetriever expects sample_index, channel_index, and segment_index
    peaks = np.zeros(
        n_spikes,
        dtype=[("sample_index", np.int64), ("channel_index", np.int64), ("segment_index", np.int64)],
    )
    ...
    print(
        f"Neighbor count stats: min={neighbor_counts.min()}, "
        f"max={max_neighbors}, mean={neighbor_counts.mean():.1f}"
    )

    # Dense padded arrays: (n_spikes, max_neighbors, n_samples)
    ...
    for i in range(n_spikes):
        ...
        wf = waveforms[i]  # (n_samples, max_neighbors)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--radius-um", type=float, default=48.0, help="Radius in microns for neighborhood (default: 48.0)"
    )
