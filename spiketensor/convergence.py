...
Usage:
    python -m spiketensor.convergence            # per-run + overlay
"""
...
def panel(out: Path, tag, s):
    h = s["history"]
    st = np.array([r["step"] for r in h], float)
    ms = np.array([r["nmse"] for r in h], float)
    wl = np.array([r["wall_s"] for r in h], float)
    ...
    for A, x, xl in ((ax[0], st, "optimizer step"), (ax[1], wl, "wall-clock (s)")):
        A.plot(x, ms, "-o", color="#4c8dff", ms=4, lw=1.6, label="minibatch nMSE")
        ...
        A.set_ylabel("reconstruction nMSE (DC-removed target)", fontsize=9)
        A.grid(alpha=.3)
...
def main():
    ...
    A = ax[2]
    for nm, col in FAM:
        sub = [(penalty_of(t)[1], s["full_nmse"]) for t, s in rows if family(t) == nm]
        if sub:
