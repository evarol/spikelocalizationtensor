...
- [All-Channel-Error Peeling (0019)](session-019-all-channel-error.md) — acceptance-bar sweep complete: 20% bar 568,888 events, 10% bar 901,334 (`16679743`), 5% bar 1,105,917 (`16679719`, pass 2 accepted 1 event, all three runs end `all_passes_complete` under the exhaustion code). Plot suite gained continuous recording replays (per-chunk, most-subtractive chunk 1580, and a two-column full-recording figure with cumulative pass rasters, `16696099`); SpikeGLX .ap.bin has 385 columns (384 AP + sync) — memmapping 384 corrupts everything. Cross-comparison with the SLT collision repo both directions: his nMSE/VE on my matched sites = 8.236/91.3% (control 8.107/85.8%); my fitter on all 2.31M of his spikes (`16685303`, 39 min): nRMSE 1.279 but captured fraction 0.421 and median worst-channel fraction 0.048, only 23.8% passing my 20% bar — his sites are weaker and less channel-concentrated than mine. 5%/10% plot galleries resubmitted (`16722284`/`16722285`) to add the replay panels the first submissions predated.
