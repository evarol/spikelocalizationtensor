...
- [All-Channel-Error Peeling (0019)](session-019-all-channel-error.md) — acceptance-bar sweep complete: 20% bar 568,888 events, 10% bar 901,334 (`16679743`, pass 2 accepted zero, ended `all_chunks_exhausted`), 5% bar 1.11M; the early-stopped 20% run was resumed under the exhaustion code as `16688480` (pass 2 at the 0.40 bar accepted exactly 1 event; final yield 568,889, `all_passes_complete`; the legacy `"stopped"` gate was removed from `pursue()` first). Plot suite gained continuous recording replays (per-chunk, most-subtractive chunk 1580, and a two-column full-recording figure with cumulative pass rasters, `16696099`); SpikeGLX .ap.bin has 385 columns (384 AP + sync) — memmapping 384 corrupts everything. Cross-comparison with the SLT collision repo: his nMSE/VE on my matched sites = 8.236/91.3% (control 8.107/85.8%); my fitter on his 2.31M spikes running as `16685303` with a self-validated harness (`0019_cross_hisspikes_fit.py`).
