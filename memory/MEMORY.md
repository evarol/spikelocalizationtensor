...
## Active
- [Acceptance-Rule Variants (023)](session-023-acceptance-rule-variants.md) — 0019's all-channel gate became a dial: `all_channel_rule` (min/mean/k-of-n) plus `all_channel_required_share` in one `all_channel_acceptance()` function with self-test coverage; four runs launched healthy 2026-09-01 evening (`16762080` flat10 no escalation, `16762081` step20 bars 0.10/0.30/0.50, `16762082` mean20 mean-channel at bar 0.2, `16762083` kofn20 7-of-8 channels at bar 0.2), each single-variable against an existing sweep run; results, quality comparison, plot-suite decision, and commit pending.
...
- [Residuals-Out Hub Index](out_hub_index.md) — `residuals/out/index.html` built by `residuals/src/plots/build_out_index.py`: one self-contained file routing to every gallery, collection, and figure under out/, with run-metadata joins and verified links.
...
## Future Work
- [Convolving Detection Peeling (024)](session-024-convolving-detection-peeling.md) — draft plan: bring back 002's temporal matched-filter proposal generator (max over Ω rows of noise-normalized residual correlation, ±5 sample × 48 µm NMS) on top of 0019's all-channel acceptance, with the proposal threshold calibrated on the never-run empirical null; motivated by the SLT cross-fit showing the threshold detector misses weak, unconcentrated sites; `--detector bipolar` must reproduce 0019 exactly.
